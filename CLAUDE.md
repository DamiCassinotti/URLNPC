...
Because the builder knows where every cover box is, `ArenaManager` also exposes `NearestCoverPoint(Vector3 from, Vector3 breakLosFrom, out Vector3 coverPoint)` (for the upcoming MoveToCover action primitive): it registers all LOS-blocking boxes at build time (everything except the floor and perimeter walls), and for each computes the point one stand-off behind the object's far side from the threat, keeping the nearest candidate that (a) sits on the NavMesh, (b) has the threat's eye-line to it blocked (eye height 1 m, matching `EnemyBehavior`'s sight ray — low geometry like stair steps self-rejects), and (c) is reachable via a complete NavMesh path from `from`. Returns `false` when the current layout offers no valid cover — callers must handle that (fall back to Retreat).
